...
module.exports = function () {
	return function (req, res, next) {
		var set_user = function (err, user) {
				...
				req.access_token = req.headers['X-ACCESS-TOKEN'] || req.query.access_token;
		...
		if (req.headers['X-ACCESS-TOKEN'] || req.query.access_token) {
			as_helper.get_info({
				access_token : req.headers['X-ACCESS-TOKEN'] || req.query.access_token,
				self : true
			}, set_user);
		}
		else {
